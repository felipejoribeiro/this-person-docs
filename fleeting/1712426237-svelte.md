...
In this case, the class `active` will be added to the `div` tag if the `isActive` variable is true. Props without default values are obligatory. You can add a `undefined` initial value, if you want it to not be obligatory but doesn't want to specify a default value.
